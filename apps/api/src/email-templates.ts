...
TripKoach Ghana Ltd · Accra · Automated lead notification.`,
  ...
  // TRI-1137 · Customer-facing acknowledgement for a custom/private date request (intent='request').
  // Carries the "within 24 hours" SLA copy (CEO #5). Sent to the requester right after the lead is saved.
  custom_date_request_received: {
    subject: 'We’ve got your date request: {{tourName}}',
    html: `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Your date request</title>
<style>
body{margin:0;padding:24px 0;background:#F1EDE6;font-family:-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#2B2724}
table{border-collapse:collapse}.w{width:600px;max-width:100%}
.mut{color:#6F675E;font-size:13px;line-height:1.5}
...
<body>
<table role="presentation" align="center" class="w" style="background:#FFFFFF;border-radius:14px;overflow:hidden">
  <tr><td style="background:#1E1C1A;padding:20px 28px"><img src="{{logoUrl}}" width="38" height="34" alt="TripKoach" style="display:block;border:0"></td></tr>
  <tr><td class="pad" style="padding:28px 28px 8px">
    <h1 style="margin:0 0 8px;font-size:22px;line-height:1.2;letter-spacing:-.02em;color:#1E1C1A">Thanks, your request is in.</h1>
    <p style="margin:0;font-size:15px;line-height:1.55">We’ve received your request for a private date on <strong>{{tourName}}</strong>. One of our koaches will review availability and get back to you <strong>within 24 hours</strong> with an indicative price and next steps.</p>
  </td></tr>
  <tr><td class="pad" style="padding:18px 28px 0">
    <table role="presentation" width="100%" style="border:1px solid #E4DFD6;border-radius:10px">
      <tr><td style="padding:11px 16px;border-bottom:1px solid #F1EDE6;font-size:14px"><span style="color:#6F675E">Tour</span><span style="float:right;font-weight:600;color:#1E1C1A;text-align:right">{{tourName}}</span></td></tr>
      <tr><td style="padding:11px 16px;border-bottom:1px solid #F1EDE6;font-size:14px"><span style="color:#6F675E">Requested date</span><span style="float:right;font-weight:600;color:#1E1C1A;text-align:right">{{requestedDate}}</span></td></tr>
      <tr><td style="padding:11px 16px;font-size:14px"><span style="color:#6F675E">Party size</span><span style="float:right;font-weight:600;color:#1E1C1A;text-align:right">{{partySize}}</span></td></tr>
    </table>
  </td></tr>
  <tr><td class="pad" style="padding:16px 28px 0"><p class="mut" style="margin:0">Pricing shared before you book is indicative and confirmed on your quote. Just reply to this email if anything changes.</p></td></tr>
  <tr><td style="background:#1E1C1A;padding:18px 28px;margin-top:20px"><p style="margin:0;color:#A8A096;font-size:12px;line-height:1.6">TripKoach Ghana Ltd · Accra</p></td></tr>
</table>
</body></html>`,
    text: `Thanks, your request is in.

We’ve received your request for a private date on {{tourName}}. One of our koaches will review availability and get back to you within 24 hours with an indicative price and next steps.

Tour: {{tourName}}
Requested date: {{requestedDate}}
Party size: {{partySize}}

Pricing shared before you book is indicative and confirmed on your quote. Just reply to this email if anything changes.

TripKoach Ghana Ltd · Accra`,
  },
  // TRI-1082: out-of-band notice to an admin whose console access was restored by another admin.
  staff_recovery: {
    subject: 'Your TripKoach console access was updated',
    html: `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>TripKoach console access updated</title>
<style>
body{margin:0;padding:24px 0;background:#F1EDE6;font-family:-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;color:#2B2724}
table{border-collapse:collapse}.w{width:600px;max-width:100%}
.mut{color:#6F675E;font-size:13px;line-height:1.5}
...
<body>
<table role="presentation" align="center" class="w" style="background:#FFFFFF;border-radius:14px;overflow:hidden">
  <tr><td style="background:#1E1C1A;padding:20px 28px"><table role="presentation" width="100%"><tr><td><img src="{{logoUrl}}" width="38" height="34" alt="TripKoach" style="display:block;border:0"></td><td align="right" style="color:#F1EDE6;font-size:13px;letter-spacing:.06em;text-transform:uppercase;font-weight:700">Account access update</td></tr></table></td></tr>
  <tr><td class="pad" style="padding:32px 28px 8px">
    <h1 style="margin:0 0 8px;font-size:22px;line-height:1.2;letter-spacing:-.02em;color:#1E1C1A">Hi {{name}},</h1>
    <p style="margin:0;font-size:16px;line-height:1.55">A change was made to your TripKoach admin console access: <strong>{{action}}</strong>.</p>
    <p style="margin:14px 0 0;font-size:16px;line-height:1.55">Performed by <strong>{{actorName}}</strong> on {{when}}.</p>
  </td></tr>
  <tr><td class="pad" style="padding:16px 28px 0"><p class="mut" style="margin:0">If you did not expect this change, contact another administrator immediately. Your account security may be at risk. This is an automated notification; you do not need to reply.</p></td></tr>
  <tr><td style="background:#1E1C1A;padding:20px 28px;margin-top:20px"><p style="margin:0;color:#A8A096;font-size:12px;line-height:1.6">TripKoach Ghana Ltd · Accra · Staff console security notice.</p></td></tr>
</table>
</body></html>`,
    text: `Hi {{name}},

A change was made to your TripKoach admin console access: {{action}}.

Performed by {{actorName}} on {{when}}.

If you did not expect this change, contact another administrator immediately. Your account
security may be at risk. This is an automated notification; you do not need to reply.

TripKoach Ghana Ltd · Accra · Staff console security notice.`,
  },
