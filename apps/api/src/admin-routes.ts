...
export function registerAdmin(app: FastifyInstance, db: Db, cfg: Config, paystack?: PaystackClient): void {
  const svc = createAdminService(db, cfg, paystack ?? createPaystackClient(cfg.paystack));
