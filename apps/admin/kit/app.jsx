...
function AdminApp() {
  ...
  const user = liveAuthed
    ? (function () {
        ...
        return { name, role: (role.charAt(0).toUpperCase() + role.slice(1)), rawRole: role, initials: s.initials || name.split(/\s+/).map(w => w[0]).join("").slice(0, 2).toUpperCase(), email: s.email || "", phone: s.phone || "", greet: (name.split(/\s+/)[0]), jobTitle: s.jobTitle || null };
      })()
